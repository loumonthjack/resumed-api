...
class ResumeService extends BaseService<'ResumeService'> {
  async enhance(userId: string, field: "bio" | "experience", arrayNumber?: number): Promise<Response> {
    ...
    const prompt = `Write the following statement in a more professional way: Statement: "${enhanceData}"`;
  ...
  async upsert(userId: string, args: Partial<Omit<ResumeType, "id" | "userId">>): Promise<Response> {
    const resumeResponse = await this.get(userId);
  ...
  async create(args: Partial<ResumeType> & { userId: string }): Promise<Response> {
  ...
  async upload(userId: string, file: string): Promise<Response> {
    ...
    if (resumeData.bio) { resume['bio'] = resumeData.bio; }
    if (resumeData.education) { resume['education'] = resumeData.education; }
    if (resumeData.experience) { resume['experience'] = resumeData.experience; }
    if (resumeData.skills) { resume['skills'] = resumeData.skills; }
    if (resumeData.awards) { resume['awards'] = resumeData.awards; }
  ...
  async update(args: Partial<ResumeType> & { userId: string }): Promise<Response> {
    const userResponse = await User.get(args.userId);
    if (!userResponse.user) return ErrorResponse(userResponse.message);
    ...
    const updatedResume = await ResumeDB.update(response.resume.id, {
      ...response.resume,
      ...args,
      bio: args.bio ? args.bio : (!response.resume.bio ? '' : response.resume.bio),
      education: args.education ? args.education : (!response.resume.education ? [] : response.resume.education),
      experience: args.experience ? args.experience : (!response.resume.experience ? [] : response.resume.experience),
      skills: args.skills ? args.skills : (!response.resume.skills ? [] : response.resume.skills),
      awards: args.awards ? args.awards : (!response.resume.awards ? [] : response.resume.awards),
